...
void main() {
      ...
      for(float i = 0.0; i < 16.0; i++) {
        ...
        float rangeCheck = smoothstep(0.0, 1.0, radius / (5.0/*max(25.0 - 0.5 * distSample, 1.0)*/ * abs(distSample - distWorld)));
        if (distSample < distWorld) {
          occluded += rangeCheck * dot(sampleDirection, normal);
        }
      }
      gl_FragColor = vec4(vec3(pow(clamp(1.0 - occluded / 16.0, 0.0, 1.0), 1.0))/* * diffuse.rgb*/, 1.0);
}`
